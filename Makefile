...
#------------------------------------------------------------------
$(TARGET)/core.lib: $(LIB_CORE_OBJECTS) | $(TARGET)/core
	cd $(TARGET)/core && ar65 a core.lib *.o
	cd $(TARGET)/core && mv core.lib $(TARGET)

#------------------------------------------------------------------
# Targets to build sources
#------------------------------------------------------------------
# I can't figure out how to make cc65 honor relative paths,
# hence the crude changing of directory and subsequent move	
$(TARGET)/%.o: $(SOURCE)/%.asm $(INCLUDES) | $(TARGET)
	cd $(dir $<) && cl65 -t cx16 -c $(notdir $<) -o $(notdir $@)
	cd $(dir $<) && mv $(notdir $@) $@
